	...
	public function index($setting) {
		...
		foreach ($categories as $category) {
			if ($category['top']) {
				...
				$totalProducts1 = $this->model_catalog_product->getTotalProducts($data_category);
				if (!$totalProducts1) {
					continue;
				}
				...
				foreach ($children as $child) {
					...
					$totalProductsChild = $this->model_catalog_product->getTotalProducts($filter_data);
					if (!$totalProductsChild) {
						continue;
					}
					...
					foreach ($children_level2 as $child_level2) {
						...
						$totalProductsChildLevel2 = $this->model_catalog_product->getTotalProducts($data_level2);
						if (!$totalProductsChildLevel2) {
							continue;
						}

						$product_total_level2 = '';
						if ($this->config->get('config_product_count')) {
								$product_total_level2 = ' (' . $totalProductsChildLevel2 . ')';
						}
