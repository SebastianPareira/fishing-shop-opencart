...
class ControllerCatalogManufacturer extends Controller {
	...
	{
		$seoUrlGenerator = new \Slug\DatabaseEntrySlug($this->db, 'manufacturer');
		$id = isset($this->request->get['manufacturer_id']) ? $this->request->get['manufacturer_id'] : null;
		...
		foreach ($this->request->post['manufacturer_seo_url'] as $store_id => $language) {
			foreach ($language as $language_id => $keyword) {
				if (empty(trim($keyword))) {
					...
					$this->request->post['manufacturer_seo_url'][$store_id][$language_id] = $keyword;
				}
			}
		}
	}
