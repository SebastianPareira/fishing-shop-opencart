	...
	protected function modifyPostInput()
	{
		foreach ($this->request->post['information_description'] as $language_id => $value) {
			if (empty($value['seo_h1'])) {
				$this->request->post['information_description'][$language_id]['seo_h1'] = $this->request->post['information_description'][$language_id]['title'];
			}

			if (empty($value['meta_title'])) {
				$this->request->post['information_description'][$language_id]['meta_title'] = $this->request->post['information_description'][$language_id]['title'];
			}
		}

		$seoUrlGenerator = new \Slug\DatabaseEntrySlug($this->db, 'information');
		$id = isset($this->request->get['information_id']) ? $this->request->get['information_id'] : null;

		foreach ($this->request->post['information_seo_url'] as $store_id => $language) {
			foreach ($language as $language_id => $keyword) {
				if (empty(trim($keyword))) {
					$keyword = $seoUrlGenerator->createSeoUrl(
						$this->request->post['information_description'][$language_id]['title'],
						$id
					);
					$this->request->post['information_seo_url'][$store_id][$language_id] = $keyword;
				}
			}
		}
	}
