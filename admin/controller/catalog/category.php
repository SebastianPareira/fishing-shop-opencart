	...
	protected function modifyPostInput()
	{
		foreach ($this->request->post['category_description'] as $language_id => $value) {
			if (empty($value['seo_h1'])) {
				$this->request->post['category_description'][$language_id]['seo_h1'] = $this->request->post['category_description'][$language_id]['name'];
			}
		...
		$seoUrlGenerator = new \Slug\DatabaseEntrySlug($this->db, 'category');
		$id = isset($this->request->get['category_id']) ? $this->request->get['category_id'] : null;

		foreach ($this->request->post['category_seo_url'] as $store_id => $language) {
			foreach ($language as $language_id => $keyword) {
				if (empty(trim($keyword))) {
					$keyword = $seoUrlGenerator->createSeoUrl(
						$this->request->post['category_description'][$language_id]['name'],
						$id
					);
					$this->request->post['category_seo_url'][$store_id][$language_id] = $keyword;
				}
			}
		}
	}
