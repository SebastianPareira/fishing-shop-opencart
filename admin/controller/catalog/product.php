...
class ControllerCatalogProduct extends Controller {
		...
		foreach ($results as $result) {
			...
			foreach ($product_specials  as $product_special) {
				if (($product_special['date_start'] == '0000-00-00' || strtotime($product_special['date_start']) < time()) && ($product_special['date_end'] == '0000-00-00' || strtotime($product_special['date_end']) > time())) {
					$special = $this->currency->format($product_special['price'], $this->config->get('config_admin_currency'));
			...
			$data['products'][] = array(
				...
				'name'       => $result['name'],
				'model'      => $result['model'],
				'price'      => $this->currency->format($result['price'], $this->config->get('config_admin_currency')),
				'special'    => $special,
	...
	protected function getForm() {
		...
		if (isset($this->error['seo_h1'])) {
			$data['error_seo_h1'] = $this->error['seo_h1'];
		} else {
			$data['error_seo_h1'] = array();
		}
		...
		$data['seo_title_suffix'] = trim($this->config->get('config_seo_title_suffix'));
		$data['seo_description_suffix'] = trim($this->config->get('config_seo_description_suffix'));
	...
	protected function validateForm() {
		...
		foreach ($this->request->post['product_description'] as $language_id => $value) {
			...
			if ((utf8_strlen($value['seo_h1']) < 1) || (utf8_strlen($value['seo_h1']) > 255)) {
				$this->error['seo_h1'][$language_id] = $this->language->get('error_seo_h1');
			}
						...
						foreach ($product_option['product_option_value'] as $product_option_value) {
							...
							if ($option_value_info) {
								$product_option_value_data[] = array(
									...
									'price'                   => (float)$product_option_value['price'] ? $this->currency->format($product_option_value['price'], $this->config->get('config_admin_currency')) : false,
									'price_prefix'            => $product_option_value['price_prefix']
								);
							}
						}
	...
	protected function modifyPostInput()
	{
		foreach ($this->request->post['product_description'] as $language_id => $value) {
			if (empty($value['seo_h1'])) {
				$this->request->post['product_description'][$language_id]['seo_h1'] = $this->request->post['product_description'][$language_id]['name'];
			}
		...
		$seoUrlGenerator = new \Slug\DatabaseEntrySlug($this->db, 'product');
		$id = isset($this->request->get['product_id']) ? $this->request->get['product_id'] : null;

		foreach ($this->request->post['product_seo_url'] as $store_id => $language) {
			foreach ($language as $language_id => $keyword) {
				if (empty(trim($keyword))) {
					$keyword = $seoUrlGenerator->createSeoUrl(
						$this->request->post['product_description'][$language_id]['name'],
						$id
					);
					$this->request->post['product_seo_url'][$store_id][$language_id] = $keyword;
				}
			}
		}
	}
