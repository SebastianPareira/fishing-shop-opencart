	...
	protected function getList() {
		...
		foreach ($results as $result) {
			...
			$data['vouchers'][] = array(
				...
				'to'         => $result['to_name'],
				'theme'      => $result['theme'],
				'amount'     => $this->currency->format($result['amount'], $this->config->get('config_admin_currency')),
				...
				foreach ($vouchers as $voucher_id) {
					...
					if ($voucher_info) {
						...
						if ($order_info) {
							...
							$data['text_greeting'] = sprintf($language->get('text_greeting'), $this->currency->format($voucher_info['amount'], (!empty($order_info['currency_code']) ? $order_info['currency_code'] : $this->config->get('config_admin_currency')), (!empty($order_info['currency_value']) ? $order_info['currency_value'] : $this->currency->getValue($this->config->get('config_admin_currency')))));
						...
						// If voucher does not belong to an order
							...
							$data['text_greeting'] = sprintf($this->language->get('text_greeting'), $this->currency->format($voucher_info['amount'], $this->config->get('config_admin_currency')));
