...
public function updatePrivatbank($default = '') {
						$currencies = array();
						...
						$this->model_localisation_currency->editValueByCode($default, $def_currency_val); 
					}
...
public function refresh() {
						...
						if ($this->validateRefresh()) {
							$this->updatePrivatbank($this->config->get('config_currency'));
	...
	protected function validateDelete() {
		...
		foreach ($this->request->post['selected'] as $currency_id) {
			...
			if ($currency_info) {
				if ($this->config->get('config_admin_currency') == $currency_info['code']) {
